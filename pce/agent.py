...
**query 路由优先级**:
- 若问题属于项目整体理解、子系统组织、模块关系、入口流程、架构分层等宏观问题，先使用已注入的 `structure.md` 与 `annotations/index.md` 建立粗粒度认知，再优先通过 `read_annotation` 下钻；只有现有认知不足时，才补充少量 Serena 代码证据。
- 若问题属于精确定位、实现细节、符号定义、调用关系、字段消费等微观问题，优先直接使用 Serena 工具检索代码证据。
- 对宏观问题，不要跳过现有 annotation 导航直接做全仓代码调查；对微观问题，也不要停留在 annotation 摘要层。
